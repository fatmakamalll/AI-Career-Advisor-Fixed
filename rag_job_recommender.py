...
Retrieved Career Options (numbered sources, already ranked best-to-worst by an
automated retrieval algorithm that combines semantic similarity and keyword
relevance -- Source 1 is the top-ranked match):
{context}
...
- Default to recommending Source 1, since it is already the top-ranked match.
  Only recommend a different source if Source 1 clearly conflicts with the
  user profile in an important way (e.g. a completely different experience
  level or industry) -- and if you do override the ranking, say so explicitly
  in "Why this matches".
