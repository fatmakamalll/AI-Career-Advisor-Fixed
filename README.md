...
## Round 4 fixes (LLM selection bias + UI restructure)

Two issues surfaced during real usage: the app always seemed to say
*"based on Source 1,"* and the results screen felt like a wall of text.

1. **`build_prompt()` now tells the model the sources are already ranked.**
   Root cause: the prompt asked the LLM to freely pick the best of 5 numbered
   sources, which is exactly the kind of task where LLMs are documented to
   exhibit **first-item / primacy bias** — defaulting to whichever option is
   listed first, independent of its actual quality. This is not a fringe
   claim; it is reported across multiple independent studies:
   - Wang et al., *"Primacy Effect of ChatGPT,"* EMNLP 2023 — ChatGPT
     systematically favored earlier-listed labels in classification tasks.
   - Raimondi & Gabbrielli (University of Bologna), *"Exploiting Primacy
     Effect to Improve LLMs,"* 2025 — found fine-tuned models show a
     **stronger** primacy bias than their base counterparts.
   - Vardasbi, Penha, Hauff & Bouchard (Spotify Research), *"Adaptive
     Repetition for Mitigating Position Bias in LLM-Based Ranking,"* 2025 —
     documents the same bias specifically in "pick the top item from a
     candidate list" ranking tasks, which is what `get_recommendation()` asks
     the model to do.

   Since `retrieve_similar_jobs()` already sorts candidates by
   `hybrid_score` before Source 1 ever reaches the prompt, Source 1 being
   picked most of the time is not necessarily wrong — but the model was never
   told *why* it should trust that ordering, so there was no way to tell
   principled ranking apart from blind position bias. The prompt now states
   explicitly that the sources are pre-ranked by the retrieval algorithm and
   instructs the model to default to Source 1, only overriding it — and
   saying so explicitly — when Source 1 clearly conflicts with the profile.
   This does not change *what* gets recommended in the typical case; it
   makes *why* auditable instead of an unexplained model habit.

2. **`app.py` parses the LLM's structured markdown instead of dumping it
   raw.** `build_prompt()` already enforces a fixed `## Heading` structure,
   so a small regex-based parser (`parse_recommendation`, `parse_bullet_list`,
   `parse_key_values`) now turns that response into actual UI: a highlighted
   role box, colored skill pills (matched vs. missing), and a 4-column info
   card for experience/industry/remote/salary — instead of one long
   `st.markdown()` text block. This follows two standard, citable usability
   heuristics from the Nielsen Norman Group: *"Recognition rather than
   recall"* (structured, scannable info beats a paragraph the user has to
   read fully) and *"Aesthetic and minimalist design."* If the model ever
   deviates from the expected format, parsing simply comes back incomplete
   and the UI falls back to the original raw-markdown rendering — it never
   breaks or shows a blank card. The "no strong match" case also now gets
   its own `st.info()` treatment instead of sharing the same card styling as
